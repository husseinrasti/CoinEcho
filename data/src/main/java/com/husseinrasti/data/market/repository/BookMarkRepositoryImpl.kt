...
class BookMarkRepositoryImpl @Inject constructor(val bookMarkDao:BookMarkCoinDataSource,val coinDao:CoinDataSource) :BookMarkRepository {
    ...
    override suspend fun addBookMark(bookmarkCoinEntity: BookmarkCoinEntity, bookMarkState: Int) {
        bookMarkDao.insertBookMarkId(bookmarkCoinEntity,bookMarkState)
        coinDao.updateBookMark(bookMarkState,bookmarkCoinEntity.id)
    }
}